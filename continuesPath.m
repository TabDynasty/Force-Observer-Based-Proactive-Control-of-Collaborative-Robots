...
alpha = 1.5; % 更新速率x
L = [5, 0, 0, 0; 0, 5, 0, 0; 0, 0, 5, 0; 0, 0, 0, 5];  %观测器反馈矩阵
xi_hat = [e ; de];  %误差估计值          
dxi_hat = [de ; dde];
...
while true
    ...
    %更新人类轨迹
    if t<10
        xh(1) = - t^3/2500 + (3*t^2)/500 + 1/5;
    elseif t>=9.5 && t<20.5
        xh(1) = (171*t)/11000 - (4779383691824715*t^2)/9223372036854775808 - (3077981411292057*t^3)/41538374868278621028243970633760768 + 5362599843569231/18014398509481984;
    elseif t>=20.5 && t<30
        xh(1) = t^3/2500 - (3*t^2)/100 + (18*t)/25 - 26/5;
    else
        xh(1) = 0.2;
    end
    ...
    sim.addForceAndTorque(endEffector, [uh(1)+d(1)+ 10*noise2, uh(2)+d(2)+ 10*noise2,0], zeros(1,3));  %施加力
...
axis()
legend('参考轨迹','当前轨迹','目标轨迹','FontName','宋体','FontSize',15);
ylim([0.15 0.5]);
xlabel('时间(s)','FontName','宋体','FontSize',15);
ylabel('x轴位置(m)','FontName','宋体','FontSize',15)
grid;
...
ylim([0.15 0.5]);
xlabel('时间(s)','FontName','宋体','FontSize',15)
ylabel('y轴位置(m)','FontName','宋体','FontSize',15)

grid;

figure(3);
plot(time, d_out(1, :),'b','linewidth',0.5);
hold on;
plot(time, d_hat_out(1, :),'r-','linewidth',0.5);
legend( '实际干扰力','估计干扰力','FontName','宋体','FontSize',15);
xlabel('时间(s)','FontName','宋体','FontSize',15)
ylabel('x轴干扰力(N)','FontName','宋体','FontSize',15)

grid;

figure(4);
plot(time, d_out(2, :),'b','linewidth',0.5);
hold on;
plot(time, d_hat_out(2, :),'r-','linewidth',0.5);
legend( '实际干扰力','估计干扰力','FontName','宋体','FontSize',15);
xlabel('时间(s)','FontName','宋体','FontSize',15)
ylabel('y轴干扰力(N)','FontName','宋体','FontSize',15)

grid;

figure(5);
plot(x_out(2, :), x_out(1, :),'b','linewidth',2);
hold on;
plot(xh_out(2, :),xh_out(1, :),'g','linewidth',2);
hold on;
plot(x_out(1,1),x_out(2,1),"b*");
hold on;
plot(x_out(2,5000),x_out(1,5000),"r*");
hold on;
plot(xh_out(2,5000),xh_out(1,5000),"g*");
legend('当前轨迹','人类目标轨迹','FontName','宋体','FontSize',15);
xlabel('y轴位置(m)','FontName','宋体','FontSize',15)
ylabel('x轴位置(m)','FontName','宋体','FontSize',15)
